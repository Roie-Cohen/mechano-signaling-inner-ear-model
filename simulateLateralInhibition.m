function g = simulateLateralInhibition(ini_latpath, latnum, with_video)
% simulates lateral inihibtion model
%% general
warning off
...
% video
if with_video
    sim_folder = 'simulations\LI\';
    vid = VideoWriter([sim_folder, 'sim_LI_', num2str(latnum),'.avi']);
...
ts = 30; t=1;
while t<= ts
    savename = ['lattices\LI\lat_', num2str(latnum),'_t=', num2str(t-1),'.mat'];
    save(savename, 'g');
    g = relaxLattice(g,9, vid, 0);
...
savename = [lattices\LI\lat_', num2str(latnum),'_final.mat'];
save(savename, 'g');
