function g = simulateHybrid1(ini_latpath, latnum, with_video)
% runs hybrid1 model starting from an initial random lattice
...
% video
if with_video
    sim_folder = 'simulations\hybrid1\';
    vid = VideoWriter([sim_folder, 'sim_hybrid1_', num2str(latnum),'.avi']);
...
ts = 100; t=1;
while t<= ts
    savename = ['lattices\hybrid1\lat_', num2str(latnum),'_t=', num2str(t-1),'.mat'];
    save(savename, 'g');
...
savename = ['lattices\hybrid1\lat_', num2str(latnum),'_final.mat'];
save(savename, 'g');
