...
for ci=1:length(cand)
    ...
    if (cellarea(ng,c)<thresh*ng.areas(c))
        if ismember(0,g.bonds(g.cells{c+1},4)), crit=0; end
        while (length(ng.cells{c+1})>3)
            if(crit==0) %% sort randomly
                id = randperm(length(ng.cells{c+1}));
            end
            if(crit==1) %% sort by boundary length
                vidx=g.bonds(ng.cells{c+1},1);
                vert = getRelativePosition(ng,vidx);
                nb=length(vidx);
                len = zeros(nb,1);
                for j=1:nb
                ...
                [ord, id] =sort(len);
            elseif (crit==2), %% sort by neighboring notch level
